...
把外部事件转成 Agent 工作的既有扩展点 `ctx.webhookRuntime` 明确不适用：它对每一次已验证投递都新建一个 root Session，从不读取回复，也不持有任何出站通道（[Webhook 子系统](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/subsystems/webhook.zh.md)）。聊天通道需要的恰好相反——每个会话一个稳定 Session，外加一条把答复送回同一聊天的路径。
...
这项能力是一个插件，装载到运维方运行的长生命周期 profile 中。它不新增应用启动器、不修改宿主 harness 的任何核心包、也不新增 `ctx` 键：每一项注册都是 effect，在该行卸载时回卷。代码位于本仓库的 `src/`，以 `@ezreal_lyy/dsh-channel-wecom` 发布。

当前只存在一个通道，因此本包把传输、策略、路由与投递放在一起。当第二个通道需要同一套 Service Definition、provider 与 consumer 角色时，`ctx.channels` 能力接缝才获得存在的理由；那时再抽取，比现在猜测它的接口更便宜。
...
| 包 | 职责 | 注册内容 |
|---|---|---|
| `@ezreal_lyy/dsh-channel-wecom` | 长连接、策略、Session 绑定、入站归一化、流式回复 | 连接器自身的 effect 树 |
...
机器人的 BotID 与 Secret 只以凭证引用的形式出现，由 `ctx.credentials` 按次解析（[credentials 包组](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/credentials/README.zh.md)）；轮换后的取值在下一条消息即生效，无需重启。

### 会话绑定

一个会话映射到由账号与会话地址确定性推导出的单一 Session 身份，因此绑定不需要旁路映射表，也能跨重启存活。收到入站消息时，连接器按该身份查找活跃 Agent，若无活跃者则恢复已持久化的 Session，否则新建：新建时把配置的工作目录作为 Session cwd，通过 `setup` 挂载 agent preset，并把可选的模型路由交给 `agentOptions`（[Agent 句柄](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/core/agent/README.zh.md)）。
...
followup 携带的消息来源会写明通道、账号、会话、发送者与提供方消息 id。连接器通过在 `MessageSourceMap` 上做声明合并来新增该变体，而该类型正是 LLM 词汇表公开的可合并扩展联合（[消息来源](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/llm/llm/src/message.ts)）。

本方案不新增任何 session event，也不改动会话格式。已发布格式迁移中的封闭来源列表描述的是**已发布世代可能包含什么**，因此新的生产者变体不属于那里（[迁移来源列表](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/session/session-format-v2-to-v3/src/payload.ts)）。
...
该包通过 `dsh plugin` 装入 profile；部署取值来自 profile 自身的 `cordis.patch.yml` 或 `--patch` 覆盖层（[插件安装](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/user/develop/basic/publish.zh.md)）。从 `web` 模板创建的 profile 无需改动代码即可同时承载浏览器界面与该通道。

该行只需要[基础 bundle](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/bundle/base/cordis.patch.yml) 已提供的服务，既不要求 HTTP 服务器，也不要求 Workspace 注册表：长连接主动向外拨号，工作目录由连接器自行提供。缺少必需服务的 profile 会在加载时报错。

### 生命周期与热插拔

在 `patchReload: live` 下，任一 patch 文件的有效编辑都会事务性重组，Loader 会对每个条目做差分：配置未变的行继续运行，被编辑的行则重启其插件 fiber（[应用引导](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/boot/app-boot/README.zh.md)）。因此连接器把卸载与重新 apply 当作正常路径：卸载时断开 socket、停止心跳、释放它创建的 Agent 句柄；重新 apply 时重连，并在下一条入站消息处恢复同一批 Session。被拒绝的编辑会让应用继续运行上一份可用配置树。
...
**给 `ctx.webhookRuntime` 增加回复通道。** 该运行时已发布的契约是即发即忘、每次投递一个 Session，且不保存投递与完成状态（[即发即忘的 webhook Session](https://github.com/deepseek-ai/deepseek-harness/blob/master/.agents/notes/implemented/feature/2026-08-22-fire-and-forget-webhook-sessions.zh.md)）。会话连续性与出站通道会给一个刻意不持有任何生命周期的包增加第二套生命周期。
...
- 本次改动不新增 session event、不提升会话格式版本。本阶段不包含自动化覆盖：该包不带单元测试或快照测试，唯一的检查是 `pnpm run build`。
